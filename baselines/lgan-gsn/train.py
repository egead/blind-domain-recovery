...
def train_lie_gan(
    ...
    for ep in trange(num_epochs):
        ...
        for i, (x, y) in enumerate(dataloader):
            ...
            G_reg_list.append(g_reg.item() / np.maximum(lamda, 1e-6))
            G_spreg_list.append(g_spreg.item() / np.maximum(mu, 1e-6))
            G_chreg_list.append(g_chreg.item() / np.maximum(eta, 1e-6))
        ...
        # [NEW] 2. Record Epoch Averages
        history['G_losses'].append(np.mean(G_loss_list))
        # Total D loss is average of Real + Fake
        d_epoch_loss = (np.mean(D_real_loss_list) + np.mean(D_fake_loss_list)) / 2
        history['D_losses'].append(d_epoch_loss)

        if save_path is not None and (ep + 1) % save_every == 0:
