...
def run_tica_experiment(exp_name, exp_specs, save_dir=None):
    ...
    N_COMPONENTS = exp_specs.get("n_components", 63)
    POOL_SIZE = exp_specs.get("pool_size", 5)
    USE_ORTHOGONAL = exp_specs.get("use_orthogonal", True)
    IS_CIRCULANT = exp_specs.get("is_circulant", True)
    LEARNING_RATE = exp_specs.get("learning_rate", 0.01)
    DET_PENALTY = exp_specs.get("det_penalty", 0.1)
    FEATURE_TYPE = exp_specs.get("feature_type", "gaussian")
    ...
    print(f"Generating {TOTAL_SAMPLES} GSN samples into memory for {exp_name}...")
    
    # Setup for Generalized Shot Noise (Gaussian basis used as default here)
    gaussian_features = [{"type": "gaussian", 
                          "scale_min": 0.5, 
                          "scale_max": 2.5, 
                          "amplitude_min": 0.5, 
                          "amplitude_max": 1.5}]
    
    legendre_features = [{"type": "legendre", 
                          "scale_min": 6.0, 
                          "scale_max": 15.0, 
                          "amplitude_min": 0.5, 
                          "amplitude_max": 1.5,
                         ...
                         {"type": "legendre", 
                          ...
                          "l":2,
                          "m":1}]
    features = gaussian_features if FEATURE_TYPE == "gaussian" else legendre_features
    ...
    # Removed the / 255.0 normalization here since GSN is not bounded pixel data
